...
-- Edge `live-activity-push` de mettre à jour l'écran verrouillé/Dynamic Island d'un appareil même
...
create policy "anon can upsert own live_activity_tokens" on public.live_activity_tokens
    for insert
    to anon
    with check (true);

create policy "anon can update own live_activity_tokens" on public.live_activity_tokens
    for update
    to anon
    using (true)
    with check (true);

create policy "anon can delete own live_activity_tokens" on public.live_activity_tokens
    for delete
    to anon
    using (true);

create index if not exists live_activity_tokens_match_id_idx on public.live_activity_tokens (match_id);
